...
Mat dehazing::convert(Mat image)
{
	IplImage *imInput;
	...
	dehazing dehazingImg(nWid, nHei, 20, false, false, 30.0f, 1.0f, 40);

	clock_t startTime = clock();
	dehazingImg.ImageHazeRemoval(imInput, imOutput);
	cout << "hehazing time: " << clock() - startTime << "ms" << endl;;
	Mat out(imOutput);
	return out;
}
...
void dehazing::HazeRemoval(IplImage* imInput, IplImage* imOutput, int nFrame)
{
	...
	test->imageData[nK] = (uchar)(m_pnSmallYImg1[nK]);
	cvNamedWindow("tests");
...
void dehazing::ImageHazeRemoval(IplImage* imInput, IplImage* imOutput)
{
	...
	//////////////////////////////////////////////////////////////////////////////////////////
	cv::Mat t_src(cv::Size(m_nWid, m_nHei), CV_8UC3);
	uchar *pdata = t_src.data;
	for (int i = 0; i < m_nHei; i++)
	{
		for (int j = 0; j < m_nWid; j++, pdata += 3)
		{
			pdata[0] = (uchar)(255 * m_pfTransmission[i*m_nWid + j]);
			pdata[1] = (uchar)(255 * m_pfTransmission[i*m_nWid + j]);
			pdata[2] = (uchar)(255 * m_pfTransmission[i*m_nWid + j]);
		}
	}

//	IplImage src_transmission(t_src);
	//cvSaveImage("src.jpg", &src_transmission);
	//////////////////////////////////////////////////////////////////////////////////////////

	clock_t startTime = clock();
	GuidedFilter(m_nWid, m_nHei, 0.001);
	printf("GuidedFilter time: %d ms\n", clock() - startTime);

	//////////////////////////////////////////////////////////////////////////////////////////
	cv::Mat t_dst(cv::Size(m_nWid, m_nHei), CV_8UC3);
	pdata = t_dst.data;
	for (int i = 0; i < m_nHei; i++)
	{
		for (int j = 0; j < m_nWid; j++, pdata += 3)
		{
			pdata[0] = (uchar)(255 * m_pfTransmissionR[i*m_nWid + j]);
			pdata[1] = (uchar)(255 * m_pfTransmissionR[i*m_nWid + j]);
			pdata[2] = (uchar)(255 * m_pfTransmissionR[i*m_nWid + j]);
		}
	}

//	IplImage dst_transmission(t_dst);
//	cvSaveImage("transmission_dst.jpg", &dst_transmission);
	//////////////////////////////////////////////////////////////////////////////////////////
